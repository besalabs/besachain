...
import { Menu, X, Wallet, ExternalLink, Droplets, Sprout, BarChart3, ArrowLeftRight } from 'lucide-react';
            ...
            <div className="flex flex-col gap-2">
              ...
              {ecosystemLinks.map((link) => (
                <a
                  ...
                  target={link.external ? '_blank' : undefined}
                  rel={link.external ? 'noopener noreferrer' : undefined}
                  className="flex items-center justify-between px-4 py-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors"
                  onClick={() => setIsOpen(false)}
                >
                  {link.label}
              ...
              <button
                onClick={handleConnect}
                className="mt-4 flex items-center justify-center gap-2 px-4 py-3 bg-emerald-500 text-black rounded-lg font-medium"
              >
                <Wallet className="w-4 h-4" />
