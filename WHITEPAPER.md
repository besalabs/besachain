...
| Account Abstraction | EIP-7702-based | EIP-7702-based |
...
Shor's algorithm (1994) demonstrated that sufficiently powerful quantum computers can factor integers and compute discrete logarithms in polynomial time, breaking RSA and ECC. Recent advances suggest cryptographically-relevant quantum computers may arrive within 10-15 years.
...
In August 2024, NIST finalized FIPS 204, standardizing ML-DSA (Module-Lattice-Based Digital Signature Algorithm), a lattice-based signature scheme secure against quantum attacks.
...
Besa implements an account abstraction mechanism based on the EIP-7702 candidate specification at the protocol level. **Note:** EIP-7702 is currently in 'Review' status and may change before finalization. Besa will update its implementation to match the final specification.
...
| Community Distribution | 10% | 6mo cliff, 12mo vest | Early builders, testnet participants |
...
6. Buterin, V. (2024). *L1/L2 Ethereum Strategy*. Ethereum Foundation Blog.

---

## Appendix A: Implementation Status

**ML-DSA Precompile (`0x0120`):**
- Status: Implemented and deployed on testnet
- Verification: Testnet transaction hash available upon request
- Audit: Security audit scheduled prior to mainnet deployment

**EIP-7702 Implementation:**
- Status: Protocol-level implementation based on EIP-7702 candidate specification
- Note: EIP-7702 is in 'Review' status; implementation will be updated to match final specification

**Chain IDs:**
- **Besa L1:** TBD at genesis (check chainlist.org for uniqueness)
- **Besa L2:** TBD at genesis

## Appendix B: Precompile Details

**ML-DSA Precompile (`0x0120`):**
- Standard: NIST FIPS 204 (ML-DSA-65)
- Operations: keygen, sign, verify
- Gas costs: ~50K (keygen), ~30K (sign), ~20K (verify)
- Testnet: Available for developer testing

**BLS12-381 Precompile (`0x0110`):**
- Purpose: BLS signature aggregation for fast finality
- Operations: pairing, aggregation, verification
- Status: Implemented, pending testnet deployment
...
- All genesis contracts will be audited prior to deployment
...
**Disclaimer:** This whitepaper is for informational purposes only and does not constitute an offer to sell or solicitation of an offer to buy any securities or investment products.
